        ...
        for data in result.get("data", []):
            ...
            for kkk in range(0, len(results[0])):
                ...
                if vals["cutting-doy"] < 270:
                    vals["exportedCutBiomass-1st-cut"] = vals.pop("exportedCutBiomass")
                else:
...
def write_row_to_grids(row_col_data, row, ncols, header, path_to_output_dir, cut_usage):
    ...
    output_grids = {
        "exportedCutBiomass-1st-cut": {"data" : make_dict_nparr(), "cast-to": "float", "digits": 2},
        "exportedCutBiomass-2nd-cut": {"data" : make_dict_nparr(), "cast-to": "float", "digits": 2}#,
...
def run_consumer(path_to_output_dir = None, leave_after_finished_run = True, server = {"server": None, "port": None, "nd-port": None}):
    ...
    if not write_normal_output_files:  
        ...
        cut_usage_to_data = defaultdict(lambda: {
